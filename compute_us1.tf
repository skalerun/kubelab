# locals {
#   users_us1 =  [
#       {
#         user = "amastour"
#         publickey = "ssh-rsa  jihadbenabra@cs-201626074710-default"
#       },
#       {
#         user = "cejjebbouji"
#         publickey = "ssh-rsa  jihadbenabra@cs-201626074710-default"
#       },
#       {
#         user = "ambeker"
#         publickey = "ssh-rsa  jihadbenabra@cs-201626074710-default"
#       },
#       {
#         user = "oguedhi"
#         publickey = "ssh-rsa  jihadbenabra@cs-201626074710-default"
#       },
      
#   ]
# }

# resource "google_compute_instance" "kube_master_us1" {
#   for_each     = {for u in local.users_us1: u.user => u}

#   name         = "kube-${each.value.user}-master"
#   machine_type = var.machine_type
#   zone         = "us-west1-b"
#   tags         = var.tags
#   boot_disk {
#     initialize_params {
#       image = var.os
#     }
#   }
#   network_interface {
#     network    = google_compute_network.kube_network.id
#     subnetwork = google_compute_subnetwork.kube_subnet_us1.id
#     access_config {}
#   }
#   metadata = {
#     ssh-keys = join("\n", [for key in local.users_us1 : "${key.user}:${key.publickey}"])
#   }
# }

# resource "google_compute_instance" "kube_worker_us1" {
#   for_each     = {for u in local.users_us1: u.user => u}
  
#   name         = "kube-${each.value.user}-worker"
#   machine_type = var.machine_type
#   zone         = "us-west1-b"
#   tags         = var.tags
#   boot_disk {
#     initialize_params {
#       image = var.os
#     }
#   }
#   network_interface {
#     network    = google_compute_network.kube_network.id
#     subnetwork = google_compute_subnetwork.kube_subnet_us1.id
#     access_config {}
#   }
#   metadata = {
#     ssh-keys = join("\n", [for key in local.users_us1 : "${key.user}:${key.publickey}"])
#   }
# }